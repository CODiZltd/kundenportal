...
class backend_auftraege(models.Model):
    ...
    auf_name=fields.Char('Mandant', related='auf_mandant.mandant')
    
    auf_mandant=fields.Many2one('backend.mandanten', string="Mandant",index=True, ondelete='restrict')
    ...
    def _get_default_currency(self):
        related_model_id = self.env['res.currency'].search([('name','=','EUR')]).id
        return related_model_id
    
    @api.multi
    def _lookup_accessid(self):
        for record in self:
            record.auf_accessid = str(record.id)
  
    #default=lambda self: self.env['Your model'].search([('file_type','=','company_user')])
    currency_id=fields.Many2one('res.currency', string="Währung", default=_get_default_currency)